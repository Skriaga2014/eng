...
def to_log(task, variants, answer, check, file):
    ...
    match TYPE:
        case 'eng' | 'ru_en':
            if TYPE == 'ru_en':
                answer = variants['word'].loc[int(answer)]
            line += f'{TYPE};{task["id"].iloc[0]};' \
                    f'{task["translate"].iloc[0]};' \
                    f'{task["word"].iloc[0]};'
        case 'ru' | 'en_ru':
            if TYPE == 'en_ru':
                answer = variants['translate'].loc[int(answer)]
            line += f'{TYPE};{task["id"].iloc[0]};' \
                    f'{task["word"].iloc[0]};' \
                    f'{task["translate"].iloc[0]};'
...
def base_update(base, base2, task, check):
    ...
    base.to_csv(BASE_FILE_RESERVE, index=False)
